function heatmapjs(){
    var list_id = document.getElementById("heatmap_id").value
    list_id = list_id.replace(/\s+/g, ",");
    //alert(list_id)
    //alert(typeof(list_id))
    var sample_id = document.getElementById("sample_id").value
    sample_id = sample_id.replace(/\s+/g, ",")
    //alert (sample_id)
    var oAjax = GetXmlHttpObject();
    //url = "http://127.0.0.1:8000/heatmap/draw_heatmap/+?+'p1+&+p2'"
    url = "http://127.0.0.1:8000/heatmap/draw_heatmap/"+"?list_id=" +list_id +"&sample_id="+sample_id;
    //alert(url)
    ...
    oAjax.onreadystatechange = function () {
        if (oAjax.readyState == 4 || oAjax.readyState == "complete") {
            if (oAjax.status == 200) {
                ...
                //make_clust('../../../static/mult_view.json');
                make_clust('../../static/json/cluster.json');
