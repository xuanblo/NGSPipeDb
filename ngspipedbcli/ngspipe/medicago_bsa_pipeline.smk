...
rule all:
    input:
        ...
        #
        # 3. mapping #
        genome_align_merge                  = join(flag_outdir, 'genome_align_merge.ok'),
        #
        # 4. call variation #
        call_variation_merge                   = join(flag_outdir, 'call_variation_merge.ok'),
        # 5. variation merge
        vcf_all_samples = join(flag_outdir, 'bsa_merge.ok'),
        #
        # 5. bas #
        bsa                               = join(flag_outdir, 'bsa_merge.ok'),
...
include: join("rules", "genome_align.smk")
include: join("rules", "call_variation.smk")
include: join("rules", "medicago_bsa.smk")