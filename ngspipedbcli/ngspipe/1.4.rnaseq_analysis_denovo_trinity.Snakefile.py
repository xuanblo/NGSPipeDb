...
rule all:
    input:
        ...
        #
        # 3. trinity #
        trinityout = expand(config["resultsDir"] + "/trinity/{sample}/trinity_out.ok", sample=SAMPLES)


onsuccess:
    print(message_success)
    shell("python ngspipe/scripts/sendmail.py -r {} -t {} -l {}".format(config['email_addr'], "success", "{log}"))

onerror:
    print(message_error)
    shell("python ngspipe/scripts/sendmail.py -r {} -t {} -l {}".format(config['email_addr'], "error", "{log}"))
...
include: join("rules", "assembly.trinity.Snakefile.py")
